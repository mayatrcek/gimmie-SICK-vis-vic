...
npm test           # rating-logic, tide-prediction + ERDDAP-client self-checks
```
...
- `lib/tides/` — harmonic tide prediction. `harmonics.ts` holds the constituent
  speeds and the 18.6-year nodal terms; `series.ts` maps each dive site to the
  gauge that stands in for it and falls back to a delay-corrected Open-Meteo
  series where no gauge fits. Coefficients live in `lib/data/tides/*.json`
  (~1 KB each) and are regenerated by `tools/fit-tides.ts`.
- `tools/fit-tides.ts` — offline: downloads years of gauge data (BOM ABSLMP for
  Lorne/Stony Point/Portland, CSIRO's Williamstown record for Port Phillip),
  least-squares fits the constituents and writes `lib/data/tides/`. Prints a
  hold-out RMS against a year that wasn't fitted — 1-4 cm vs the Bureau's own
  predictions. Caches downloads in `tools/.cache/` (gitignored).
